...
Tools & Diagnostics:
  ...
  docs        Print the complete gopm reference (agent-friendly)
...
The self-contained capability reference. Compiled into the binary as data, so
the reference always matches the binary that prints it — there is no generation
step and no file to ship alongside.
...
- No arguments — print every topic, preceded by a header naming the binary
  version and topic count. This is the AI-agent ingest path: one call yields the
  full capability surface.
- One or more topic names — print those sections, in the order requested.
  Lookup is case-insensitive; an unknown name exits 1 and points at `--list`.
- `--list` — print the index grouped by section.
- `--json` — `{version, topics: [...]}`, where each topic carries
  `name, group, command, summary, body, usage, aliases, flags, examples, notes,
  see_also`. The version field lets a caller invalidate a cached copy after an
  upgrade.

Coloring is on only when stdout is a character device, so redirected and piped
output is plain text. `NO_COLOR` disables it in `auto` mode; `--color always`
and `--color never` override the detection.

Topic groups mirror the `gopm --help` sections, preceded by a **Concepts &
Formats** group covering the model rather than the commands:

| Group | Topics |
|-------|--------|
| Concepts & Formats | overview, global-flags, targets, restart-policies, ecosystem, config, logging, files, environment, exit-codes, json-output, mcp, automation |
| Process Management | start, stop, restart, delete, list, describe, isrunning, isprocess, logs, flush, watch, stats |
| Daemon Management | ping, status, kill, reboot, install, uninstall, suspend, unsuspend |
| Configuration & State | export, import, resurrect, pm2 |
| Tools & Diagnostics | gui, pid, version, docs |

`help` and `completion` are generated by cobra rather than declared in this
codebase; they are described in the `overview` topic instead of having their
own.

**Maintenance contract.** The reference is only useful while it is true, and
agents act on it without a human in the loop. Every new command, flag, config
key, exit code, status value, or behavior change is reflected in the docs
topics in the same commit that introduces it:

| File | Contents |
|------|----------|
| `internal/cli/docs.go` | Data model, topic assembly, renderer |
| `internal/cli/docs_guides.go` | Concepts & Formats topics |
| `internal/cli/docs_commands_process.go` | Process Management topics |
| `internal/cli/docs_commands_daemon.go` | Daemon Management topics |
| `internal/cli/docs_commands_tools.go` | Configuration & State and Tools topics |

`internal/cli/docs_test.go` turns the contract into a failing test rather than
a convention: it walks the real cobra command tree and fails when a registered
command has no topic, when a visible flag is undocumented, when a documented
flag no longer exists (allowing for build-tagged commands such as `pid`), when
a shorthand disagrees, or when a `See also` names a topic that does not exist.
